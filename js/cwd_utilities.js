...
(function ($, root, undefined) { $(function () { 'use strict';
	...
	function resizeChecks() {
		...
		if (window.matchMedia('(max-width: 991px)').matches) {
			$('#sidebar-top > nav > div > ul > li').addClass('mobile-appendage').prependTo( $('#main-navigation #menu-top-menu') );
			if($('.mobile-appendage.parent .fa-caret-down').length < 1 ) {
				$('.dropdown-menu li.mobile-appendage.parent > a').append('<span class="fa fa-caret-down"></span>'); // wrap text in a span and add dropdown caret icons
			}
		}
		else {
			$('#main-navigation .mobile-appendage').removeClass('mobile-appendage').prependTo('#sidebar-top > nav > div > ul');
		}
	...
	$('#main-navigation li.parent').parent().removeClass('menu').addClass('links list-menu');
	$('nav li.parent > a').append('<span class="fa fa-caret-down"></span>'); // wrap text in a span and add dropdown caret icons
	$('nav li.parent li.parent > a .fa').removeClass('fa-caret-down').addClass('fa-caret-right'); // change sub-dropdown caret icons
	...
	$('nav li.parent li.parent > ul').removeAttr('style'); // reset min-width to allow smaller submenus
	...
	$('nav li.parent a').focus(function() {
		if ( !$('body').hasClass('mobile') ) {
			// horizontal edge-detection
	...
	// Mobile Navigation
	$('nav li.parent > a .fa').click(function(e) {
		e.preventDefault();
		e.stopPropagation();
	}).mousedown(function(e) {
		e.stopPropagation();
